...
import { blockTitle, requireNode } from "../views.ts";

import { buildBlockView, type BlockView, type Line } from "./block-view.ts";
...
import {
  ...
  siblingColumns,
  siblingListTitle,
} from "./sibling-table.ts";
import { sourceLines, trimCommonIndent } from "./source-lines.ts";
import { pieceText, preview } from "./values.ts";

import type { SiblingCell } from "./sibling-table.ts";

import type { RecordedValue, Trace, TraceNode } from "@codewalk/trace";
...
function blockNodes(trace: Trace, title: string): TraceNode[] {
  return trace.nodes.filter(
    ({ loc }) => loc.role === "block" && loc.title === title,
  );
}

function titleAmong(blocks: TraceNode[], block: TraceNode): string {
  return blockTitle(block, blocks.indexOf(block), blocks.length);
}

function rowValue(line: Line, label: string): RecordedValue | null {
  return (
    line.after.find((row) => row.label.endsWith(label))?.values[0]?.entry ??
    null
  );
}
...
describe("buildBlockView", () => {
  test("a nested iteration body is inert in its parent while the loop header stays lit", () => {
    ...
    const view = buildBlockView(trace, requireNode(trace, 0), []);
  ...
  test("a while condition belongs to each iteration, including the final failed check", () => {
    ...
    const parent = buildBlockView(trace, requireNode(trace, 0), []);
  ...
  test("a clause header is lit only in the executions that entered its body", () => {
    ...
    const root = requireNode(trace, 0);

    const headerStates = (block: TraceNode, number: number) =>
  ...
  test("an activation dims the branch it skipped and exposes the nested call it ran", () => {
    const trace = fixture("fact");
    const view = buildBlockView(trace, requireNode(trace, 16), []);
  ...
  test("token, statement, and nested site boundaries all split spans without losing site order", () => {
    ...
    const header = {
      codewalk: 3,
      literals: {},
      source: { file: "boundaries.py", text: "abcdefgh" },
      locs: [
        blockLoc("boundaries.py", 0, 8, null),
        { role: "stmt", start: 0, end: 6, parent: 0 },
        { role: "expr", start: 1, end: 5, parent: 1 },
        { role: "expr", start: 2, end: 4, parent: 2 },
        blockLoc("outer", 1, 5, 2),
        blockLoc("inner", 2, 4, 3),
        { role: "stmt", start: 6, end: 8, parent: 0 },
      ],
    };

    const events = [
      ...[0, 1, 2, 3, 5].map((loc) => ({ op: "enter", loc })),
      ...Array.from({ length: 2 }, () => ({ op: "exit" })),
      { op: "enter", loc: 4 },
      ...Array.from({ length: 4 }, () => ({ op: "exit" })),
      { op: "end", status: "ok" },
    ];

    const parsed = parseTrace(
      [header, ...events].map((line) => `${JSON.stringify(line)}\n`).join(""),
    );
    ...
    expect(
      buildBlockView(trace, requireNode(trace, 0), [
  ...
  test("values stay in their owning block and follow the exact anchor span", () => {
    ...
    const moduleView = buildBlockView(trace, requireNode(trace, 0), []);
  ...
  test("an iteration's inputs sit on its first line and each change on the line that made it", () => {
    ...
    expect(line(search, 3).inputs.map(({ name }) => name)).toEqual([
      "lo",
      "hi",
      "items",
      "target",
  ...
  test("a loop's end state follows its body at the header's indent, before any else clause", () => {
    ...
    if (search === undefined) throw new Error("Missing loop fixtures");

    const moduleView = buildBlockView(whileLoop, requireNode(whileLoop, 0), []);
        ...
        .flatMap(({ number, changes, after }) => [
          ...
          ...after
            ...
            .flatMap(({ label, values }) =>
              values.map(
                ({ name, entry }) =>
                  `${number}: ${JSON.stringify(label)} ${name} → ${shown(trace, entry)}`,
              ),
            ),
        ]);

    expect(loopEnds(whileLoop, moduleView)).toEqual(['4: "(after)" i → 2']);
  ...
  test("chip lines group only with adjacent chip lines, and a loop's end row ends the group", () => {
    const groups = (trace: Trace) =>
      buildBlockView(trace, requireNode(trace, 0), []).groups.map((group) =>
        group.length === 1
  ...
  test("an exception is marked where it was raised and where it was caught, not where it passed through", () => {
    ...
    expect(
      line(buildBlockView(uncaught, requireNode(uncaught, 0), []), 5).exception,
    ).toBeNull();
    ...
    const module = buildBlockView(caught, requireNode(caught, 0), []);
  ...
  test("a return value sits on its return line in the block that ran it, as it was when returned", () => {
    ...
    const returned = (block: TraceNode, number: number) => {
      const value = rowValue(
        line(buildBlockView(trace, block, []), number),
        "(returned)",
      );

      return value === null ? null : shown(trace, value);
    };
  ...
  test("titles use trace labels, site indexes, and units", () => {
    ...
    const root = requireNode(fact, 0);

    expect(titleAmong([root], root)).toBe("fact.py");
    expect(titleAmong(iterations, secondIteration)).toBe("iteration 2");
    expect(titleAmong([firstFunction], firstFunction)).toBe("fact");
    expect(siblingListTitle(iterations)).toBe("2 iterations");

    const callbacks = fixture("native_callback");
    const callbackBlocks = blockNodes(callbacks, "key");

    expect(
      callbackBlocks.map((block) => titleAmong(callbackBlocks, block)),
    ).toEqual(["key 1", "key 2"]);
  });
  ...
  test("sibling columns keep only the values that differ between siblings, including ones some siblings lack", () => {
    ...
    expect(
      lastIterations.map((block) =>
        cellTexts(
          siblingCells(trace, block, siblingColumns(trace, lastIterations)),
        ),
      ),
    ).toEqual([
      ["3", null],
...
test("a value its statement writes as a literal gets no chip yet still ends a loop's state", () => {
  ...
  const module = buildBlockView(trace, requireNode(trace, 0), []);

  const returned = (block: TraceNode, number: number) =>
    rowValue(line(buildBlockView(trace, block, []), number), "(returned)");
